...
function generateHeaderComment(
	...
	let fourthLine = replaceCharacterAtIndex(
		theLine,
		5,
		`Problem Number: ${headerInfo.problemNumber}`
	);
	...
	let sixthLine = replaceCharacterAtIndex(
		theLine,
		5,
		`By: ${headerInfo.author} <${headerInfo.authorUrl}>`
	);
	...
	let ninthLine = replaceCharacterAtIndex(
		theLine,
		5,
		`Solved: ${year}/${month}/${day} ${hours}:${minutes}:${seconds} by ${headerInfo.author}`
	);
