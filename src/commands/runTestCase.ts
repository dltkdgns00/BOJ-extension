...
export async function runTestCase(context: vscode.ExtensionContext) {
    ...
    try {
        ...
        const preMessage1 = centerText(` ${problemNumber}. ${sp.title} `, maxWidth);
        ...
        const createOutputMessage = (testCaseIndex, result, expected, actual) => {
            ...
            const failedMessage = `❌ Test Case #${testCaseIndex + 1}: Failed\n${centerText(
                ` Expected `,
                maxWidth
        ...
        const runTest = (input, expectedOutput, testCaseIndex) => {
            return new Promise<void>((resolve, reject) => {
                ...
                processIO!.on("close", (code) => {
                    ...
                    const message = createOutputMessage(testCaseIndex, isPassed, expectedOutput, actualOutput);
        ...
        vscode.window.showErrorMessage("테스트 케이스 실행 중 오류가 발생했습니다. 직접 실행해서 오류를 확인해주세요.");
    }
}
...
function getProcessIO(extension: string, filePath: string) {
    if (extension === "c") {
        ...
        execSync(`javac -d ${projectDir} "${filePath}" `);
        return spawn("java", ["-cp", `${projectDir}`, "Main"]);
        ...
        execSync(`rustc --crate-name "${crateName}" "${filePath}" -o "${objectFileURL}"`, options);
